...
unsigned char MY[10];
unsigned static int dest=0,del=0;  // setting Destination address as 0 and Delay of 1 sec
...
void loop() {                                     //main code to run repeatedly
  ...
  delay(1000);                                    //delay of 1.9 sec.
    ...
    if(flag==1)                                   //a check to excecute the Gyroscope sensor function
      {
        gyro();                                   //excecute the Gyroscope sensor function
      }

    if(flag==2)                                   //a check to excecute the Temperature sensor function
      {
        gyro_tem();                               //excecute the Temperature sensor function
      }    
}
...
void serialEvent1() {                                       //Function to receive a character or string as input
  while (Serial1.available()) {                             //if any incoming data is available in Serial1
    ...
    if(inChar=='&'||inChar=='@'||inChar=='!'||inChar=='$'||inChar=='0'||inChar=='1'||inChar=='2'||inChar=='3'||inChar=='*'||inChar=='#'||inChar=='4'||inChar=='5'||inChar=='6'||inChar=='7'||inChar=='8'||inChar=='9')
...
void choose()
{
    ...
    checkst = inputString.substring(1,2);           //checking out error in the received command at 1st position
    if(startID=="#"||startID=="0"||startID=="1"||startID=="2"||startID=="3"||startID=="4"||startID=="*"||checkst=="&"||checkst=="@"||checkst=="!"||checkst=="$") 
    {
      ...
      nodeID=inputString.substring(2,4);            //storing the next characters at 2nd and 3rd position from the string to get its node ID
      sensorID=inputString.substring(5,7);          //storing the next characters at 5th and 6th position from the string to get its sensor ID
    ...
    if(startID=="$")                                          // to change the destination address of a node
     {
         if(nodeID =="03"||nodeID=="00")                       //to check if its for NODE 3
         {
    ...
    if(nodeID=="03"||nodeID=="00")
      ...
      {
         if(nodeID=="03"||nodeID=="00")                                       //to check if its for NODE 2
          {
...
void sread()                                                    //Function to initiate or change sensor
{
    ...
    {
        amtsend--;
        if(sensorID=="01")                                      //check for Gyroscope sensor
           { 
             flag=1;
           } 
        else if(sensorID=="02")                                 //check for Temperature sensor
            { 
             flag=2;
            }
    }
...
char gyro()
{
        ...
        GyX=Wire.read()<<8|Wire.read();                     //reading sensor vaue
        ...
        String s2 = ",3, "+hr+":"+minit+":"+sec+", \"Data\":["+String(pitch,2);
        String s3 = "], ["+String(roll,2);
        String s4 = "], ["+String(yaw,2)+"]}##";          
        String jsonString = s1+s2+s3+s4;                      //concatenating the individual strings
...
char gyro_tem()
{
        ...
        int len = jsonString.length();                        //finding string length
        char charBuff[len];
        jsonString.toCharArray(charBuff,len-1);               //converting string to array of characters
        if (flagch==0)
        {
